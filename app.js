document.addEventListener("DOMContentLoaded", () => {
  ...
  window.switchApp = function(app) {
    ...
    if (app === 'kinto') {
      kintoEl.style.display = 'block'; bisEl.style.display = 'none';
      btnKinto.classList.add('active'); btnBis.classList.remove('active');
      calculateKinto();
    } else {
      kintoEl.style.display = 'none'; bisEl.style.display = 'block';
      btnKinto.classList.remove('active'); btnBis.classList.add('active');
      calcBis();
    }
  };
  ...
  window.addArtwork = function(value = "") {
    ...
    input.type = "number"; input.placeholder = "幅"; input.value = value;
    input.inputMode = "decimal"; input.className = "art-width-input";
    input.oninput = calculateKinto;
    const groupLabel = document.createElement("label");
    groupLabel.className = "group-check-label";
    const groupCheck = document.createElement("input");
    groupCheck.type = "checkbox"; groupCheck.className = "group-check";
    groupCheck.onchange = calculateKinto;
    groupLabel.appendChild(groupCheck); groupLabel.append("連");
    const del = document.createElement("button");
    del.innerText = "×"; del.type = "button";
    del.onclick = () => { wrapper.remove(); lockedGaps = {}; calculateKinto(); };
    wrapper.appendChild(input); wrapper.appendChild(groupLabel); wrapper.appendChild(del);
  ...
  function calculateKinto() {
    ...
    if (!wall) return;
    const mode = calcMode.value, edgeVal = parseFloat(document.getElementById("edgeWidth").value) || 0;
    const midGapVal = parseFloat(document.getElementById("midGapWidth").value) || 0;
    const groupGapVal = parseFloat(groupGapInput.value) || 0, slideOffset = parseFloat(slideOffsetText.value) || 0;
    const rows = artworksDiv.querySelectorAll(".art-input-row"), artworks = [], groupFlags = [];
    rows.forEach(row => {
    ...
    const totalArtWidth = artworks.reduce((a, b) => a + b, 0), gapCount = artworks.length + 1;
    ...
    for (let i = 0; i < artworks.length - 1; i++) if (groupFlags[i] && groupFlags[i+1]) gapsState[i+1] = { value: groupGapVal, isFixed: true };
    Object.keys(lockedGaps).forEach(idx => { const i = parseInt(idx); if (i < gapCount) gapsState[i] = { value: lockedGaps[idx], isFixed: true }; });
    const fixedSum = gapsState.reduce((sum, g) => sum + (g.isFixed ? g.value : 0), 0), freeIndices = gapsState.map((g, i) => g.isFixed ? null : i).filter(v => v !== null);
    if (freeIndices.length > 0) { const freeVal = (wall - totalArtWidth - fixedSum) / freeIndices.length; freeIndices.forEach(idx => gapsState[idx].value = freeVal); }
    let finalGaps = gapsState.map(g => g.value); finalGaps[0] += slideOffset; finalGaps[gapCount - 1] -= slideOffset;
  ...
  function renderIndividualGaps(gaps, isFixedArray) {
    ...
    gaps.forEach((g, i) => {
      ...
      const input = document.createElement("input"); input.type = "number"; input.value = g.toFixed(1);
      ...
      row.appendChild(input); individualGapsDiv.appendChild(row);
    });
  }
  ...
  function drawKinto(wall, artworks, gaps, groupFlags) {
    ...
    const scale = canvas.width / wall; let x = 0;
    gaps.forEach((gap, i) => {
      x += gap * scale;
      if (i < artworks.length) {
        const w = artworks[i] * scale, inGroup = (groupFlags[i] && groupFlags[i+1]) || (groupFlags[i] && groupFlags[i-1]);
        ctx.fillStyle = inGroup ? "#007bff" : "#333"; ctx.fillRect(x, 40, w, 40);
        ctx.fillStyle = "white"; ctx.fillText(String.fromCharCode(65 + i), x + 4, 65);
        ctx.fillStyle = "black"; ctx.fillText(`${artworks[i]}`, x, 35);
        ctx.fillStyle = "blue"; ctx.fillText(`${gap.toFixed(0)}`, x - (gap * scale) / 2 - 8, 105);
        x += w;
      } else { ctx.fillStyle = "blue"; ctx.fillText(`${gap.toFixed(0)}`, canvas.width - (gap * scale) / 2 - 8, 105); }
    });
  }
  ...
  window.calcBis = function() {
    ...
    if (h > 0) {
      resA = (center - h / 2).toFixed(0); // 床から底面
      resB = (center + h / 2).toFixed(0); // 床から天面
      if (fromTop > 0) {
        const bisH = parseFloat(resB) - fromTop;
        resD = bisH.toFixed(0); // 床からビス高さ
        resC = (bisH - center).toFixed(0); // センターからビス
      }
    }
    if (w > 0) {
      let e, f;
  ...
  function drawBis(center, h, w, bisC, bisE, bisF, preset, isActualWidth) {
    const canvas = document.getElementById("canvas-bis"), ctx = canvas.getContext("2d");
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    if (h <= 0) return;
    ...
    ctx.strokeStyle = "#999"; ctx.beginPath(); ctx.moveTo(10, groundY); ctx.lineTo(canvas.width - 10, groundY); ctx.stroke();
    ctx.fillStyle = "#999"; ctx.fillText("床", 5, groundY + 12);
    ctx.setLineDash([2, 2]); ctx.strokeStyle = "rgba(255,0,0,0.5)"; ctx.beginPath(); ctx.moveTo(10, centerY); ctx.lineTo(canvas.width-10, centerY); ctx.stroke();
    ctx.setLineDash([]); ctx.fillStyle = "red"; ctx.font = "bold 11px sans-serif"; ctx.fillText(`CL: ${center}`, 10, centerY - 7);

    ctx.strokeStyle = isActualWidth ? "#333" : "#ccc"; ctx.lineWidth = 2; ctx.strokeRect(cx - rw/2, ty, rw, rh);
    const bisY = centerY - (bisC * scale);

    // 高さ指標 (右側に移動して重なりを回避)
    drawHelper(ctx, cx + rw/2 + 15, groundY, cx + rw/2 + 15, by, "#666", `A:${(center - h/2).toFixed(0)}`);
    drawHelper(ctx, cx + rw/2 + 48, groundY, cx + rw/2 + 48, ty, "#666", `B:${(center + h/2).toFixed(0)}`);
    
    // ビス高さ指標 (左側)
    drawHelper(ctx, 40, groundY, 40, bisY, "#d9534f", `D:${(center + bisC).toFixed(0)}`);
    if (Math.abs(bisC) > 2) drawHelper(ctx, cx - rw/2 - 15, centerY, cx - rw/2 - 15, bisY, "red", `C:${bisC > 0 ? '+' : ''}${bisC}`);
    ...
    if (isActualWidth && preset !== "1:1") {
      ...
      drawHelper(ctx, blx, bisY, brx, bisY, "blue", `F:${bisF}`);
      drawHelper(ctx, cx - rw/2, ty + 15, blx, ty + 15, "#007bff", `E:${bisE}`);
